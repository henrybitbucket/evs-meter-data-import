package com.pa.evs.constant;

public interface RestPath {
    String API = "/api";

	String LOGIN = API + "/login";
	String LOGIN1 = API + "/v1/login";
	String WHOAMI = API + "/user/me";
	String WHOAMI1 = API + "/whoami";
	
    String USER_USERNAME = API + "/user/username.php";
    String USER_USER_DETAILS = API + "/user/user_details.php";
	String USERS = API + "/users";
	String USER = API + "/user";
	String USER_PERMISSION = API + "/user/permission";
	String USER_ROLE_LOGGING = API + "/user/role/logging";
	String USER_PROJECT_LOGGING = API + "/user/project/logging";
	String USER_ROLE = API + "/user/role";
	String USER_PROJECT = API + "/user/project";
	String USER_COMPANY_LOGGING = API + "/user/company/logging";
	String USER_COMPANY = API + "/user/company";
	String USER_PLATFORM = API + "/user/pf";
	String USER_GROUP = API + "/user/group";
	String EACHUSERPERMISSION = API + "/each-user/permission";
	String UPDATEROLE = API + "/update/role";
	String UPDATEGROUP = API + "/update/group";
	String UPDATEPERMISSON = API + "/update/permission";
	String UPDATEPROJECT = API + "/update/project";
	String UPDATECOMPANY = API + "/update/company";
	String CREATE_NEW_USER = API + "/user/save";
	String CREATE_NEW_ACCESS_PERMISSION = API + "/dms/nus-access-key";
	String GET_ACCESS_PERMISSIONS = API + "/dms/nus-access-keys";
	String CREATE_NEW_ACCESS_PERMISSION_FROM_EMAIL = API + "/dms/nus-access-keys-email";
	
	String CA_REQUEST_LOG_GET = API + "/ca-request-logs";
	String CA_REQUEST_LOG = API + "/ca-request-log";
	String CA_REQUEST_LOG_GET_CIDS = API + "/ca-request-log/cids";
	String CA_REQUEST_LOG_POST_SEND_COMMAND = API + "/ca-request-log/command";
	String CA_REQUEST_LOG_BATCH_COUPLE_DEVICES = API + "/ca-request-log/batch-couple";
    String CA_REQUEST_LOG_GET_COUNT_DEVICES = API + "/ca-request-log/count-devices";
    
	String RLS_STATUS_GET_LOGS = API + "/rls-command-logs";
	String DASHBOARD_CA_CAL = API + "/cal-dashboard";
	String DASHBOARD_CA_ALARM_MARK_VIEW_ALL = API + "/alarm-mark-view-all";
    String DASHBOARD_GET = API + "/dashboard";
    String GET_DMS_MC_USERS = API + "/dms-mc-users";
    String SAVE_DMS_MC_USERS = API + "/dms-mc-user";
    String GET_VENDOR_AND_MC_USERS = API + "/vendor-users";
    String DELETE_VENDOR_AND_MC_USERS = API + "/vendor-users/{vendorId}";
    String SAVE_OR_UPDATE_VENDOR_AND_MC_USERS = API + "/vendor-users/save-or-update";
    String GET_PAGIN_VENDORS = API + "/vendors-users";
}
