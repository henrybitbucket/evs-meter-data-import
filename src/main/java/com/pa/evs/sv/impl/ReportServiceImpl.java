...
public class ReportServiceImpl implements ReportService {
	...
	@Override
	public void getMeterFileDatas(PaginDto<MeterFileDataDto> pagin) {
		
		Object piId = pagin.getOptions().get("piId");
		
		StringBuilder sqlBuilder = new StringBuilder("FROM MeterFileData");
        StringBuilder sqlCountBuilder = new StringBuilder("SELECT count(*) FROM MeterFileData");

        StringBuilder sqlCommonBuilder = new StringBuilder();
        sqlCommonBuilder.append(" WHERE 1=1 ");
        sqlCommonBuilder.append(" AND pi.id = " + piId);
        ...
        List<MeterFileData> list = query.getResultList();

        list.forEach(li -> {
        	MeterFileDataDto dto = MeterFileDataDto.builder()
                    .id(li.getId())
                    .filename(li.getFilename())
                    .ftpResStatus(li.getFtpResStatus())
