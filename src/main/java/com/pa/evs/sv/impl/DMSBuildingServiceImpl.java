...
import com.pa.evs.model.DMSBuilding.BuildingType;
...
@Transactional
@Service
public class DMSBuildingServiceImpl implements DMSBuildingService {
	...
	public void save(BuildingDto dto) throws ApiException {
		if (dto.getId() != null) {
			...
			DMSBuilding entity = new DMSBuilding();
			DMSAddress address = new DMSAddress();
			...
			entity.setType(com.pa.evs.model.DMSBuilding.BuildingType.from(dto.getType()));
			entity.setAddress(address);
				...
				for(BlockDto blockDto : blocks) {
					...
					if(Objects.nonNull(blockDto.getLevels())) {
						...
						for(FloorLevelDto floorDto : levels) {
							DMSFloorLevel floorLevel = new DMSFloorLevel();
							...
							if(Objects.nonNull(floorDto.getUnits())) {
								...
								for(BuildingUnitDto buildingUnitDto : units) {
									DMSBuildingUnit buildingUnit = new DMSBuildingUnit();
	...
	public void search(PaginDto<BuildingDto> pagin, String search) {
		...
		if ((exportCsv || detailUnit)) {
			...
			sqlBuilder.append(" ,bl.name blName, fl.name fName, bu.name buName, b.id bId, fl.id fId, bu.id buId, bu.remark, bu.coupled_date, bu.modify_date bu_modify_date, bu.location_tag ");
		} else {
		...
		sqlBuilder.append(" from {h-schema}dms_building b ");
		if ((exportCsv || detailUnit)) {
			sqlBuilder.append(" left join {h-schema}dms_floor_level fl on (fl.building_id = b.id) ");
		...
		if (StringUtils.isNotBlank(queryMsn)) {
			sqlBuilder.append(" and (LOWER((SELECT crl1.msn FROM {h-schema}ca_request_log crl1 WHERE crl1.building_unit_id = bu.id AND crl1.building_id = b.id LIMIT 1)) like '%" + queryMsn.toLowerCase() + "%')");
		}
		if (StringUtils.isNotBlank(querySn)) {
			sqlBuilder.append(" and (LOWER((SELECT crl2.sn FROM {h-schema}ca_request_log crl2 WHERE crl2.building_unit_id = bu.id AND crl2.building_id = b.id LIMIT 1)) like '%" + querySn.toLowerCase() + "%')");
		}
		...
		sqlCountBuilder.append(" from {h-schema}dms_building b ");
		if ((exportCsv || detailUnit)) {
			sqlCountBuilder.append(" left join {h-schema}dms_floor_level fl on (fl.building_id = b.id) ");
		...
		if (StringUtils.isNotBlank(queryMsn)) {
			sqlCountBuilder.append(" and (LOWER((SELECT crl1.msn FROM {h-schema}ca_request_log crl1 WHERE crl1.building_unit_id = bu.id AND crl1.building_id = b.id LIMIT 1)) like '%" + queryMsn.toLowerCase() + "%')");
		}
		if (StringUtils.isNotBlank(querySn)) {
			sqlCountBuilder.append(" and (LOWER((SELECT crl2.sn FROM {h-schema}ca_request_log crl2 WHERE crl2.building_unit_id = bu.id AND crl2.building_id = b.id LIMIT 1)) like '%" + querySn.toLowerCase() + "%')");
		}
		if (StringUtils.isNotBlank(queryBuilding)) {
...
//				address.setCoupleState((o[19] + "_" + o[20] + "_" + o[21]).equals(crlId) ? "Y" : "N");
				...
				address.setLocationTag((String) o[25]);
	...
	public void update(BuildingDto buildingDto) throws ApiException {
		...
		DMSBuilding building = buildingRepository.findById(buildingDto.getId())
				.orElseThrow(() -> new ApiException(ResponseEnum.BUILDING_NOT_FOUND));

		AddressDto addressDto = buildingDto.getAddress();
		DMSAddress address = addressRepository.findById(addressDto.getId()).orElse(new DMSAddress());
		address.setCity(addressDto.getCity());
		address.setCountry(addressDto.getCountry());
		address.setDisplayName(addressDto.getDisplayName());
		address.setBlock(addressDto.getBlock());
		...
		List<DMSBuildingUnit> units = new ArrayList<>();
		List<DMSBlock> blocks = blockRepository.findAllByBuilding(building);
		List<DMSFloorLevel> levels = floorLevelRepository.findAllByBuilding(building);
		
		for (DMSFloorLevel lvl : levels) {
			List<DMSBuildingUnit> listUnit = buildingUnitRepository.findAllByFloorLevel(lvl);
