...
public interface GroupUserRepository extends JpaRepository<GroupUser, Long> {
	...
	@Modifying
	@Query("DELETE UserGroup WHERE id in (SELECT id from UserGroup WHERE user.userId = ?1 and groupUser.id not in (?2))")
	void deleteNotInGroups(Long userId, Set<Long> groupIds);
