...
public interface CARequestLogRepository extends JpaRepository<CARequestLog, Long> {
	...
	@Query(value = "select count(id) from {h-schema}log where mid is not null and topic <> 'evs/pa/local/data/send' and rep_status = -999 and type = 'PUBLISH' and (mark_view is null or mark_view <> 1)", nativeQuery = true)
	Number countAlarms();
